...
{
    /// <summary>
    /// Runs all migrations against a real, freshly-reset PostgreSQL test-database and generates the PostgreSQL entity-relationship diagram of the resulting
    ...
    /// </summary>
    [TestClass]
    public class DatabaseStructureTestsForPostgreSQL
    {
        private DatabaseTestFrameworkForPostgreSQL? _DatabaseTestFramework;

        [TestInitialize]
        public void TestInitialize()
        {
            this._DatabaseTestFramework = OpenDMSBackend.Tests.TestUtilities.Utilities.GetDatabaseTestFrameworkForPostgreSQL();
        }
        ...
        public void GenerateDatabaseStructureDiagram()
        {
            ...
            {
                DatabaseTestFrameworkForPostgreSQL databaseTestFramework = this._DatabaseTestFramework!;
                ...
                string targetFile = Path.Combine(OpenDMSBackend.Tests.TestUtilities.Constants.GeneralConstants.CodeUnitFolder, "Other", "Reference", "ReferenceContent", "Articles", "Diagrams", "DatabaseStructure.PostgreSQL.plantuml");

                //act
                string plantUml = DatabaseStructureDiagramGenerator.Generate(codeunitSpecificDatabaseInteractor, ServerLog.GetTransientLog().Logger, "OpenDMSBackend - Database structure (PostgreSQL)", targetFile);
