...
| D006 | 2025-11-17 | Timestamps will be stored in UTC in the persistence | Storing timezones in the database is tricky and to have a consistent way of storing timestamp-values correctly timestamps will be converted to UTC and then the application which display the value can convert it the timestamp back to the user's local time zone which seems to be the best practice to handle time zones. |
