...
- ✅ Provide role- and permission-management. Administrators only have administrative access; they are not automatically allowed to change (or retrieve) content. Each storage-location has a moderator (its owner) who decides who may retrieve and change its contents; by default nobody (not even an administrator) may retrieve or change the contents except the users a moderator has explicitly granted view- or edit-permission. Administrators manage global roles (a user can have multiple roles) via an admin user-overview in the UI.
