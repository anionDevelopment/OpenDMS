...
- ❌ [Allow creating custom document-attributes which every document optionally has. Supported attribute-types: bool (for example "tax-relevant" yes/no), double, timestamp (for example a deadline) and string (free-text).](https://github.com/anionDevelopment/OpenDMS/issues/2) (Partially implemented: a moderator of a storage-location can define custom metadata-fields of type string or bool for that storage-location and every contained document can optionally hold a value for each of them. The storage-location-view of the web-UI shows these fields and lets a moderator add and remove them, and the page of a document lets the value of every field be set and cleared. The types double and timestamp are not supported yet.)
...
- ✅ Provide tagging and indexing with metadata: a document can be indexed with tags (named and colored labels which exist installation-wide and can be assigned to arbitrarily many documents) and with the custom metadata-fields of its storage-location (for example document-type or contact/sender). Both can be changed in the web-UI on the page of a document and every change is audit-logged. The retention-date is not a metadata-field but is held by `DeleteIsNotAllowedBefore` and `MustBeHardDeletedAfter` (see the issues 9 and 10), and searching over tags and metadata is [issue 15](https://github.com/anionDevelopment/OpenDMS/issues/15). The open points of this feature are listed in the section [Feature 14: tagging and indexing with metadata](#feature-14-tagging-and-indexing-with-metadata).
...
The feature [tagging and indexing with metadata](https://github.com/anionDevelopment/OpenDMS/issues/14) is implemented in the scope which was agreed for it: a document can be indexed with installation-wide tags and with the custom metadata-fields of its storage-location. A moderator of a storage-location defines its fields (name and type, currently `string` or `bool`), and every document contained in that storage-location can optionally hold one value per field. The document-type and the contact/sender, which the issue names as examples, are modelled as such custom fields and not as fixed standard-fields, so that every installation can name and structure them as it needs them. The retention-date is deliberately not a metadata-field: it is held by the document-properties `DeleteIsNotAllowedBefore` and `MustBeHardDeletedAfter`.

The following points of this topic are still open. The first three are tracked in an issue of their own, the last three are not tracked anywhere yet.

- Setting the retention-date (`DeleteIsNotAllowedBefore` and `MustBeHardDeletedAfter`) is not possible yet: there is no API for it (see [issue 9](https://github.com/anionDevelopment/OpenDMS/issues/9)) and the deletion-lock during the retention-period is not enforced technically (see [issue 10](https://github.com/anionDevelopment/OpenDMS/issues/10)).
- The metadata-field-types `double` and `timestamp` do not exist; only `string` and `bool` are supported (see [issue 2](https://github.com/anionDevelopment/OpenDMS/issues/2)).
- Searching over tags and metadata is not possible; the indexed data can currently only be read on the page of a document (see [issue 15](https://github.com/anionDevelopment/OpenDMS/issues/15)).
- A tag can be created and assigned, but it can neither be renamed nor deleted afterwards, so a tag created by mistake stays in the installation forever. There is also no distinction between installation-wide tags managed by an administrator and tags which belong to a single user (a corresponding note exists in `Tag.cs`).
- A metadata-field cannot be renamed and its type cannot be changed after it was defined. The only possible change is to remove the field, which removes the values of all documents for it as well.
- Moving a document into another storage-location keeps the values it holds for the fields of its previous storage-location. Those values are no longer shown and can no longer be changed, because only the fields of the current storage-location are offered for a document, but they remain stored and become visible again when the document is moved back.
