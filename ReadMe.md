...
- ❌ [Allow creating custom document-attributes which every document optionally has. Supported attribute-types: bool (for example "tax-relevant" yes/no), double, timestamp (for example a deadline) and string (free-text).](https://github.com/anionDevelopment/OpenDMS/issues/2) (Partially implemented in the backend: a moderator of a storage-location can define custom metadata-fields of type string or bool for that storage-location and every contained document can optionally hold a value for each of them. The types double and timestamp are not supported yet.)
